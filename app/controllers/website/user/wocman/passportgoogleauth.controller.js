                          ...
                          .then((hasSettings) => {
                            if (parseInt(hasSettings.securityipa, 10) == 0) {
                              if (parseInt(hasSettings.security2fa, 10) == 0) {
                                ...
                                var unboard = Helpers.returnBoolean(
                                  xnewuser.unboard
                                );
                                var isEmailVerified = Helpers.returnBoolean(
                                  xnewuser.verify_email
                                );
                                var isProfileUpdated = Helpers.returnBoolean(
                                  xnewuser.profileupdate
                                );
                                var isCertificateUploaded = Helpers.returnBoolean(
                                  xnewuser.certificatesupdate
                                );
                                var isSkilled = Helpers.returnBoolean(
                                  xnewuser.isSkilled
                                );
                                ...
                                let response = {
                                  body: {
                                    ...
                                    intro:
                                      "Welcome to Wocman Technology! You requested an OTP to login.",
                                    action: {
                              ...
                              var joyresult = schemaJoiIP.validate({
                                ipaddress: ip,
                              });
                              var { value, error } = joyresult;
                              if (!(typeof error === "undefined")) {
                                var msg = Helpers.getJsondata(
                                  error,
                                  "details"
                                )[0];
                                ...
                                IpBlacklist.findOne({
                                  ...
                                  .then((findip) => {
                                    if (!findip) {
                                      //assuming the ip addres does not exit before now
                                      IpBlacklist.create({
                                        //we create it
                                        ip: ip,
                                        ipmode: 0,
                                      ...
                                      IpBlacklist.findOne({
                                        ...
                                        //we sent the opt to the users email
                                        // console.log(newfinhd5ip.ipmode);
  
                                        if (
                                          parseInt(newfinhd5ip.ipmode, 10) == 1
                                        ) {
                                    ...
                                    let response = {
                                      body: {
                                        name: xnewuser.username,
                                        intro:
                                          "Welcome to Wocman Technology! You are trying to login into your account from another device.",
                                        action: {
                                          instructions: `Copy this OTP to continue Login: ${otp}`,
                                          button: {},
                                        },
                                    ...
                                    let message = {
                                      ...
                                      to: xnewuser.email,
                                      subject:
                                        "Securiy Concern: Device Verification",
                                      html: mail,
                                    };
                                    ...
                                    return res.status(202).send({
                                      ...
                                      isdevice: true,
                                      message:
                                        "You are logging into your account from another device",
                                      data: {
                                        otp: otp,
                                        email: email,
                                        password: password,
                ...
                .then((hasSettings) => {
                  if (parseInt(hasSettings.securityipa, 10) == 0) {
                    if (parseInt(hasSettings.security2fa, 10) == 0) {
                      ...
                      transporter.sendMail(message).then((sentMails) => {
                        var sentMail = true;
                      });
                      User.update(
                        {
